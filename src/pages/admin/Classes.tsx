...
const contraindicationBlock = (extras: string[] = [], bullet = "•") =>
  `Do not practice this session if you have:\n${[...CORE_CONTRAINDICATIONS, ...extras].map((c) => `${bullet} ${c}`).join("\n")}`;

const additionalConditionsBlock = (items: string[], bullet = "•") =>
  `Additionally, only practice if you:\n${items.map((c) => `${bullet} ${c}`).join("\n")}`;

const buildTemplate = (...parts: string[]) => [...parts, SAFETY_FOOTER].join("\n\n");
