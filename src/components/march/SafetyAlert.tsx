      ...
      <AlertDescription className="space-y-4">
        ...
        <div className="space-y-2 rounded-lg bg-background/50 p-4">
          {CRISIS_RESOURCES.map((resource, index) => (
            <div key={index} className="flex items-start gap-3 text-sm">
              {resource.phone && <Phone className="mt-0.5 h-4 w-4 flex-shrink-0" />}
              {resource.text && <Heart className="mt-0.5 h-4 w-4 flex-shrink-0" />}
        ...
        {!isHighRisk && (
          <p className="mt-4 text-xs text-muted-foreground">
            Is there anything else I can help you with today in terms of your wellbeing goals or
            practice?
          </p>
        )}
      </AlertDescription>
