...
export const runImagePerformanceTest = async (): Promise<PerformanceReport> => {
  ...
  const report: PerformanceReport = {
    ...
    averageLoadTime:
      loadedResults.length > 0 ? Math.round(totalLoadTime / loadedResults.length) : 0,
...
export const compareReports = (before: PerformanceReport, after: PerformanceReport): void => {
  ...
  const loadTimePercent =
    before.averageLoadTime > 0
      ? ((loadTimeImprovement / before.averageLoadTime) * 100).toFixed(1)
      : "0";

  const sizeReduction = before.totalSize - after.totalSize;
  const sizePercent =
    before.totalSize > 0 ? ((sizeReduction / before.totalSize) * 100).toFixed(1) : "0";

  console.log(
    `Average Load Time: ${before.averageLoadTime}ms → ${after.averageLoadTime}ms (${loadTimePercent}% ${loadTimeImprovement > 0 ? "faster" : "slower"})`
  );
  console.log(
    `Total Size: ${(before.totalSize / 1024 / 1024).toFixed(2)}MB → ${(after.totalSize / 1024 / 1024).toFixed(2)}MB (${sizePercent}% ${sizeReduction > 0 ? "smaller" : "larger"})`
  );
  console.log(
    `Slow Images (>500ms): ${before.imagesOver500ms.length} → ${after.imagesOver500ms.length}`
  );
  console.log(
    `Large Images (>100KB): ${before.imagesOver100KB.length} → ${after.imagesOver100KB.length}`
  );
